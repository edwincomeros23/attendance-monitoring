...
try {
  ...
  if (!is_dir($uploadsDir)) {
      if (!@mkdir($uploadsDir, 0755, true)) {
          throw new Exception('Server write restricted (Render). Upload skipped.');
      }
  }
  if (!is_writable($uploadsDir)) {
      throw new Exception('Directory is not writable (Render). Upload skipped.');
  }
  ...
  if (!@move_uploaded_file($file['tmp_name'], $dest)) throw new Exception('Failed to save file (Server restricted)');
